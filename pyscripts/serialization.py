...
pargs = parser.parse_args("--sites 4".split())
...
systemcreation_code = """
projector = HubbardProjector_sp(pargs.sites, pargs.sites // 2, pargs.sites - (pargs.sites // 2))
...
metastore("everything", "systemcreation_code", "recorder", "pargs")

def reconstruct_system(data):
    pargs = data["pargs"]
    exec(data["systemcreation_code"])
    pass
data = pickle.load(open("everything.pkl", "rb"))
reconstruct_system(data)