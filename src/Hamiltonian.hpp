...
fourth_oder_timeordered_exponential(const std::function<MatrixType(double)> & H,
  ...
  MatrixType fac1 = matrix_exponential_taylor<MatrixType>(-1.0i * (c1 * H1 + c2 * H2) * dt, 4);
  MatrixType fac2 = matrix_exponential_taylor<MatrixType>(-1.0i * (c2 * H1 + c1 * H2) * dt, 4);
  return fac1 * fac2;
}
...
fourth_oder_timeordered_exponential(const std::function<MatrixType(double)> & H,
  ...
  vec_t apply_fac2 = apply_matrix_exponential_taylor<MatrixType>(fac2, state, 4);
  vec_t apply_fac1 = apply_matrix_exponential_taylor<MatrixType>(fac1, apply_fac2, 4);
  return apply_fac1;
}
