...
system_dimension = elec_dim * dimension
state_distro = coherent_photon_state(temperature, dimension) + HubbardGroundState(sites, hopping, hubbardU, periodic, projector) #+ HubbardNeelState_sp(sites, projector)

recorder = StateObservableRecorder()
...
import time
start = time.time()
Solvers.direct_closed_observable(system.system_hamiltonian(), state_distro.draw(), t, dt, recorder)
mid = time.time()
